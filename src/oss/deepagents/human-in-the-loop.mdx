...
- **`InterruptOnConfig(...)`**: Fine-grained control over decisions and conditional interrupt logic
...
`InterruptOnConfig` gives you precise control over how interrupts behave for a specific tool. You can restrict which decisions a human can make, add a description to the approval prompt, and conditionally skip the interrupt entirely based on the tool call arguments.
