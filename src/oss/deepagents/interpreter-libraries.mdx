...
Interpreter libraries ship with `@langchain/quickjs` and can be added to any agent with [interpreter middleware](/oss/deepagents/interpreters). For background on how libraries work, see [Interpreter libraries](/oss/deepagents/interpreters#interpreter-libraries).

## Swarm

The `swarm` library provides parallel task fan-out with a table-based data model. It gives agents a three-function API (`create`, `run`, `rows`) that handles concurrency, batching, error grouping, and result merging.
