...
`deepagents deploy` packages your agent configuration and deploys it as a [LangSmith Deployment](/langsmith/deployment). The command bundles the agent harness—prompts, skills, subagents, MCP servers, memory templates, and sandbox configuration—and runs it on the LangSmith Deployment runtime for durable execution, streaming, human-in-the-loop, auth, observability, cron, and open protocol endpoints.
