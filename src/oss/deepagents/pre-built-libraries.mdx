...
Prebuilt libraries ship with `@langchain/quickjs` and can be added to any agent with [interpreter middleware](/oss/deepagents/interpreters). For background on how libraries work, see [Interpreter libraries](/oss/deepagents/interpreters#interpreter-libraries).
