...
:::python
<Note>
For direct backend callers, `FilesystemBackend.ls()` returns `entries=None` with an `error` when the path does not exist, is not a directory, or cannot be resolved. An empty directory returns `error=None` and `entries=[]`, so callers can distinguish failures from empty listings. The agent-facing `ls` tool handles these errors through `FilesystemMiddleware`.
</Note>
:::
...
- `ls(path: str) -> LsResult`
  - Return entries with at least `path`. Include `is_dir`, `size`, `modified_at` when available. Sort by `path` for deterministic output. On failure, return `LsResult(error=..., entries=None)`. Return `entries=[]` only when the directory exists and is empty.
