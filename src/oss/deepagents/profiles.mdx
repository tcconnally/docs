...
<ResponseField name="system_prompt_suffix" type="string | Callable[[ProfileSuffixContext], string | None]">
    Append text to the assembled base prompt (`SUFFIX` in [Prompt assembly](/oss/deepagents/customization#prompt-assembly)); applied to the main agent, declarative subagents, and the auto-added general-purpose subagent. Use the callable form for [conditional suffix segments](/oss/deepagents/customization#conditional-suffix-segments) that depend on the final middleware stack.
</ResponseField>
...
For YAML/JSON-backed workflows, use `HarnessProfileConfig`. It mirrors the declarative subset of `HarnessProfile` (literal prompt text, tool-description overrides, excluded tools and middleware, general-purpose subagent edits) and owns `to_dict` / `from_dict`. Runtime-only state — middleware instances, factories, callable `system_prompt_suffix` resolvers, and class-form `excluded_middleware` entries — stays on `HarnessProfile`.
