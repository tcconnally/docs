...
:::python
`create_deep_agent` is a pre-assembled harness—@[`create_agent`] plus a stack of middleware covering execution environment, context management, delegation, and steering. The parameters below are the toggles for each piece.

| Category | Parameters | What it controls |
|---|---|---|
| **Execution environment** | `tools=`, `backend=`, `permissions=` | Tools, filesystem backend, and path permissions |
| **Context management** | `memory=`, `skills=`, `middleware=` | Persistent memory, skills, and extra middleware |
| **Delegation** | `subagents=` | Ephemeral subagents for isolated subtasks |
| **Steering** | `interrupt_on=` | Human approval gates |
| **Model & persona** | `model=`, `system_prompt=` | Model selection and system prompt |
| **Output** | `response_format=` | Structured output schema |
| **Harness-wide tweaks** | [profiles](#profiles) | Per-model defaults as a reusable bundle |

To compose a fully custom harness from scratch, see [Agent harness](/oss/langchain/harness).
...
| **Context management** | `memory`, `skills`, `middleware` | Persistent memory, skills, and extra middleware |
| **Delegation** | `subagents` | Ephemeral subagents for isolated subtasks |
| **Steering** | `interruptOn` | Human approval gates |
| **Model & persona** | `model`, `systemPrompt` | Model selection and system prompt |
| **Output** | `responseFormat` | Structured output schema |
