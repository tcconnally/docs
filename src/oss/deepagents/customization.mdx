...
### Conditional suffix segments

Pass a callable `system_prompt_suffix` when some suffix copy only makes sense if a middleware survives profile assembly. Deep Agents evaluates the callable after `excluded_middleware` filters the stack, so the returned text can track the final middleware classes instead of naming missing tools.

This is the pattern used by the built-in Codex profile: plan-hygiene guidance is only appended when `TodoListMiddleware` is still present, because that middleware provides `write_todos`.

```python
from langchain.agents.middleware import TodoListMiddleware

from deepagents import HarnessProfile, ProfileSuffixContext


def codex_suffix(ctx: ProfileSuffixContext) -> str:
    suffix = "Use tools efficiently."
    if TodoListMiddleware in ctx:
        suffix += "\n\nBefore finishing, reconcile TODOs with `write_todos`."
    return suffix


profile = HarnessProfile(system_prompt_suffix=codex_suffix)
```

Callable suffixes are runtime-only. `HarnessProfileConfig.system_prompt_suffix` stays `str | None` because config files cannot serialize Python callables.
