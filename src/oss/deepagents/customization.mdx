...
:::python
`create_deep_agent` is a pre-assembled harness—[`create_agent`][langchain.agents.create_agent] plus a stack of middleware covering execution environment, context management, delegation, and steering. The parameters below are the toggles for each piece.

| Category | Parameters | What it controls |
|---|---|---|
| **Execution environment** | `tools=`, `backend=`, `permissions=` | Domain tools, virtual filesystem or sandbox, path-level access control |
| **Context management** | `memory=`, `skills=`, `middleware=` | `AGENTS.md` injection, skill disclosure, summarization |
| **Delegation** | `subagents=` | Ephemeral subagents and background tasks |
| **Steering** | `interrupt_on=`, `checkpointer=` | Human approval gates and state persistence |
| **Model & persona** | `model=`, `system_prompt=` | Which model to use and the agent's instructions |
| **Output** | `response_format=` | Structured response schema |
| **Harness-wide tweaks** | [profiles](#profiles) | Per-model prompt overrides, tool exclusions, middleware additions |

To compose a fully custom harness from scratch with `create_agent` and individual middleware, see [Agent harness](/oss/langchain/harness).

For the full parameter list, see the @[`create_deep_agent`] API reference.
:::

:::js
`createDeepAgent` is a pre-assembled harness—`createAgent` plus a stack of middleware covering execution environment, context management, delegation, and steering. The parameters below are the toggles for each piece.
...
| **Context management** | `memory`, `skills`, `middleware` | `AGENTS.md` injection, skill disclosure, summarization |
| **Delegation** | `subagents` | Ephemeral subagents and background tasks |
| **Steering** | `interruptOn`, `checkpointer` | Human approval gates and state persistence |
| **Model & persona** | `model`, `systemPrompt` | Which model to use and the agent's instructions |
| **Output** | `responseFormat` | Structured response schema |
...
`create_deep_agent` pre-assembles a middleware stack on top of @[`create_agent`]. To build a fully custom agent—choosing exactly which capabilities to include—see [Agent harness](/oss/langchain/harness).

:::

:::js

`createDeepAgent` pre-assembles a middleware stack on top of `createAgent`. To build a fully custom agent—choosing exactly which capabilities to include—see [Agent harness](/oss/langchain/harness).
