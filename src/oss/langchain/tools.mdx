...
<Tabs>
  ...
  <Tab title="Runtime tool registration">
    ...
    This approach is best when:
    - Tools are discovered at runtime (e.g., from an [MCP server](/oss/python/langchain/mcp))
...
LangChain supports the [Model Context Protocol (MCP)](https://modelcontextprotocol.io), an open standard for connecting agents to external tools and data sources. With MCP, you can load tools from any MCP-compatible server — including local processes, remote APIs, and third-party tool providers — without writing custom integration code.

<Card title="Model Context Protocol (MCP)" icon="plug" href="/oss/python/langchain/mcp">
  Connect your agents to MCP servers to access a broad ecosystem of tools. See the full guide for setup, configuration, and examples.
</Card>
