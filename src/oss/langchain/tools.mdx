...
- Return a list of [content blocks](#return-multimodal-content) for multimodal output such as images, audio, video, or PDFs.
...
Return a list of [content blocks](/oss/langchain/messages#multimodal) when your tool produces images, audio, video, PDFs, or a mix of text and media. Each block is appended to the resulting `ToolMessage` in order, so you can interleave text and media in a single tool call.
...
def render_chart(metric: str) -> list[dict]:
    """Render a chart for the given metric and return it as text plus an image."""
    png_bytes = generate_chart(metric)
    return [
...
- Each block is forwarded to the model as part of the `ToolMessage` content.
- Supported block types include `text`, `image`, `image_url`, `json`, `document`, `file`, `search_result`, and `custom_tool_call_output`.
- Multiple blocks of the same or different types can be mixed in a single response—provider permitting, the model sees them in the order you return them.

Use this when the tool output is naturally visual or includes both narration and media (for example, rendered charts, screenshots with captions, or extracted PDF pages).

<Note>
`audio` and `video` blocks are not in the default `ToolMessage` allow list. Returning them as a plain list causes them to be JSON-stringified. To send audio or video back to the model, construct a @[`ToolMessage`] explicitly with `content_blocks=[…]`:
...
def fetch_recording(call_id: str, runtime: ToolRuntime) -> ToolMessage:
    """Fetch a call recording and return it as an audio content block."""
...
Provider support varies—check the [model integration page](/oss/integrations/providers) before relying on audio or video tool returns.
</Note>
