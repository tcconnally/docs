...
Setting the same `retry_policy`, `timeout`, `cache_policy`, or `error_handler` on every @[`add_node`] call gets repetitive. Use @[`StateGraph.set_node_defaults`] to declare these once for the whole graph. Per-node values passed to `add_node` always take precedence, and defaults are applied at @[`StateGraph.compile`] time:
...
def fallback_handler(state: State, error: NodeError) -> Command:
    return Command(update={"status": f"recovered from {error.node}"}, goto="finalize")
...
In this graph, `node_a` inherits all three defaults, `node_b` uses its own `retry_policy` but inherits the default `timeout` and `error_handler`, and `finalize` inherits all three but is only reached via the handler's `Command(goto=...)`.

### What applies to error-handler nodes

`retry_policy` and `timeout` defaults apply to **every** node, including error-handler nodes — a stuck or transiently failing handler should be retried or cancelled like any other node. `cache_policy` and `error_handler` defaults apply only to **regular** nodes:

| `set_node_defaults()` kwarg | Falls back for `add_node(...)` kwarg | Applies to error-handler nodes? |
|---|---|---|
| `retry_policy` | `retry_policy` | Yes |
| `timeout` | `timeout` | Yes |
| `cache_policy` | `cache_policy` | No — caching a handler result is unsafe because the input (failed-node state) may differ across failures even when the cache key matches |
| `error_handler` | `error_handler` | No — handlers must never catch themselves |

If a handler raises, the run fails. The default handler is never invoked to recover another handler.

### Precedence

For each node, the value used at compile time is:

1. The value passed to `add_node(..., retry_policy=...)` (or `cache_policy=`, `error_handler=`, `timeout=`), if any.
2. Otherwise, the corresponding value from `set_node_defaults(...)`, if any.
3. Otherwise, no policy.

`set_node_defaults()` is chainable and can be called before or after `add_node` — defaults are not applied until `compile()`.

<Warning>
Defaults set via `set_node_defaults()` are **not** inherited by subgraphs. Set defaults on each `StateGraph` you build, or pass the policies directly to nodes that wrap subgraphs.
</Warning>
