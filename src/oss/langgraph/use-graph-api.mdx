...
Use @[`StateGraph.set_node_defaults`] to set `retry_policy`, `timeout`, `cache_policy`, or `error_handler` once for every node in a graph, instead of repeating them on each @[`add_node`] call. Per-node values always win, and defaults are applied at @[`StateGraph.compile`] time:
...
See [Fault tolerance](/oss/langgraph/fault-tolerance#graph-wide-defaults) for the full precedence rules and applicability table.
