// :snippet-start: workflows-agents-tool-runtime-state-context-js
...
// :remove-start:
const expected = "User user_123 in organization org_456";
const messageContents = result.messages.map((message) =>
  String(message.content ?? ""),
);
if (!messageContents.some((content) => content.includes(expected))) {
  throw new Error(
    `Expected "${expected}" in messages: ${JSON.stringify(messageContents)}`,
  );
}
console.log("✓ workflows-agents tool runtime state/context sample completed");
// :remove-end:
