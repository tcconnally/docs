...
Store authorization differs from threads and assistants. Handlers rewrite the mutable `namespace` field in `value` to scope data per user rather than returning metadata filters. For a walkthrough, see [Isolate store per user](/langsmith/store-auth).
