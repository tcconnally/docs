...
Reference for every input variable exposed by the [Azure Terraform modules](https://github.com/langchain-ai/terraform/tree/main/modules/azure). Set non-sensitive variables in `infra/terraform.tfvars`. Sensitive variables (license key, passwords, encryption keys) are written to `infra/secrets.auto.tfvars` by `make setup-env` and stored in Azure Key Vault by Terraform.
...
These flags are read by `init-values.sh` and `deploy.sh`. Terraform ignores them; they only affect which Helm add-on overlay files are generated.
