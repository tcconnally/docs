...
AWS, Azure, and GCP paths are fully documented. The OpenShift walkthrough lands here in a future release; until then, follow the README in the [OCP module directory](https://github.com/langchain-ai/terraform/tree/main/modules/ocp).
...
<Warning>
In-cluster ClickHouse is for development and POC only. Production deployments must use [LangChain Managed ClickHouse](/langsmith/langsmith-managed-clickhouse) or a self-managed external ClickHouse cluster. Blob storage is always required because trace payloads must not live in ClickHouse.
</Warning>
