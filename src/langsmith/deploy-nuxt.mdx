...
It is a Vue/Nuxt port of the streaming-cookbook `react-custom-backend` example: the same protocol, served from Nitro instead of a standalone Hono process, and consumed with [`@langchain/vue`](https://www.npmjs.com/package/@langchain/vue) composables instead of React hooks.
