...
Once configured, all `ClaudeSDKClient` operations will be automatically traced to LangSmith, including:
...
<Warning>
  The LangSmith integration works automatically when using `ClaudeSDKClient`. If you use the `query` API directly on the Agent SDK (without `ClaudeSDKClient`), tracing will **not** be applied automatically. Use `ClaudeSDKClient` to ensure your traces appear in LangSmith.
</Warning>
