...
An example app that deploys a LangChain **deep agent** inside a [SvelteKit](https://svelte.dev/docs/kit/introduction) project, built for [Cloudflare Workers](https://svelte.dev/docs/kit/adapter-cloudflare) with `@sveltejs/adapter-cloudflare`.

It mirrors the Next.js example: a streaming chat UI, subagent detail views, thread history, and the Agent Streaming Protocol exposed under `/api/threads/...`. No separate backend process is required.
...
Out of the box, the agent uses an in-memory `MemorySaver` checkpointer (`src/lib/server/agent/index.ts`). The per-thread SSE replay/session log lives in a Durable Object (`src/lib/server/durable-objects/thread-session.ts`) so streaming clients reconnect to one coordination point instead of a process-local map.
