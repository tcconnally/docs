...
By default, a task runs in a single container built from its `environment/Dockerfile`, and the agent and verifier run inside that one container. A multi-container task instead runs several containers at once for the same trial.

This is per trial, not per task: a trial is one agent attempt on one task, and "multi-container" describes the environment that trial runs in, not splitting the task across machines. Use it when a task needs more than the agent's own container, for example a database, a web server, or a second host the agent talks to.

To define one, include an `environment/docker-compose.yaml` file in the task that declares each container as a Compose service. The agent and verifier run in the service named `main`, and the other services are auxiliary. All services share one Compose network, so the agent reaches the others by their service names, for example a `db` service at the hostname `db`.

For each trial, Harbor brings up the whole Compose stack, runs the agent in `main`, runs the verifier, then tears every container down together. See the [Harbor sandbox documentation](https://harborframework.com/docs/run-jobs/cloud-sandboxes) for details.
