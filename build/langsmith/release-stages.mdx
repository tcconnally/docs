...
| Stage | Availability | Stability | Self-hosted / BYOC | Support |
|-------|-------------|-----------|----------------------|---------|
| [Alpha](#alpha) | Design partners only | Breaking changes expected; the feature may change significantly or never ship | Not guaranteed | No SLA |
| [Beta](#beta) | Public or a private group | Breaking changes possible; GA expected within a few months | Not guaranteed, but typically added during beta | No SLA |
| [GA](#ga) | Public | Stable; breaking changes require a deprecation period | Supported | Fully supported |
...
- **Stability**: Breaking changes are expected. The feature may change significantly, and it may never ship as GA.
...
Beta makes a functional feature available more broadly while its interface and behavior are finalized.
...
- **Stability**: The feature is functional end to end rather than a partial implementation. Breaking changes are still possible, but GA is expected within a few months.
- **Pricing**: Pricing may be available, but is not yet finalized.
- **Deployments**: Not guaranteed in self-hosted or BYOC environments, though support for those environments is typically added during beta to stabilize the feature.
- **Support**: No SLA applies.
...
- **Stability**: API behavior is stable. Breaking changes require a deprecation period.
