...
The model is the reasoning engine. The **harness** is everything around it—tools, context management, memory, subagents, and guardrails—that turns the model into an agent that can act, remember, and coordinate over long tasks. `create_agent` starts with a minimal harness and extends it through middleware. See [Agent harness](/oss/javascript/langchain/harness) for the full composable system.
