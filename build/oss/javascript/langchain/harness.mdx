...
[`create_agent`](https://reference.langchain.com/javascript/langchain/index/createAgent) is the minimal harness—a model calling tools in a loop. You extend it by adding [middleware](/oss/javascript/langchain/middleware/overview): each middleware hooks into one stage of the loop and adds one capability without changing the harness itself.
