...
For the full parameter list, see the [`createDeepAgent`](https://reference.langchain.com/javascript/deepagents/types/CreateDeepAgentParams) API reference. To compose a fully custom harness from scratch, see [Middleware capabilities](/oss/javascript/langchain/agents#middleware-capabilities).
...
`createDeepAgent` pre-assembles a middleware stack on top of `createAgent`. To build a fully custom agent—choosing exactly which capabilities to include—see [Middleware capabilities](/oss/javascript/langchain/agents#middleware-capabilities).
