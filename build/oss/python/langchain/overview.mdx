...
[LangChain](/oss/python/langchain/agents) gives you `create_agent` — a minimal harness you configure and extend with middleware. Full control, composable primitives, take exactly what your use case needs.

[Deep Agents](/oss/python/deepagents/overview/) (`create_deep_agent`) pre-assembles a production-ready harness on top of LangChain: filesystem, summarization, subagents, and prompt caching included by default. Start here when you want common patterns pre-built.

[LangGraph](/oss/python/langgraph/overview) is our low-level orchestration runtime for advanced workflows combining deterministic and agentic steps.
