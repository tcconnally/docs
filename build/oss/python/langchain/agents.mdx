...
**agent = model + harness.** A harness is everything around that loop: the model, its prompt, its tools, and any middleware that shapes its behavior. The job of a harness is to get the model the right context at the right time for the given task.
