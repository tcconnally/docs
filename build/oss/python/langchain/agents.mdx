...
You configure it through two levers:

- **Arguments**: `model=`, `tools=`, `system_prompt=`, and `response_format=` cover the most common configuration directly on `create_agent`. See [Core components](#core-components).
- **Middleware**: for more advanced capabilities — filesystem access, context management, delegation, guardrails — each middleware bundles prompts, tools, and loop logic into a composable unit. See [Configure the harness](#configure-the-harness).
...
<Note>
Many capabilities below (summarization, memory, skills, subagents) require a filesystem backend to store and retrieve data. Add [`FilesystemMiddleware`](https://reference.langchain.com/python/deepagents/middleware/filesystem/FilesystemMiddleware) to your stack to enable them. See [Backends](/oss/python/deepagents/backends).
</Note>
