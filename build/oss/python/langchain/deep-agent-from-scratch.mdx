...
```python
# analysis-skills/pandas-patterns/SKILL.md
```
...
agent = create_agent(
    ...
    middleware=[
        FilesystemMiddleware(backend=backend),
        SummarizationMiddleware(model=model, backend=backend),
        SkillsMiddleware(backend=backend, sources=["./analysis-skills/"]),
    ],
)
```
...
Before sharing results, pause for human review. [`HumanInTheLoopMiddleware`](https://reference.langchain.com/python/langchain/agents/middleware/human_in_the_loop/HumanInTheLoopMiddleware) intercepts specific tool calls and waits for approval before proceeding.
...
agent = create_agent(
    ...
    middleware=[
        ...
        TodoListMiddleware(),
        SubAgentMiddleware(backend=backend, subagents=[visualizer]),
        HumanInTheLoopMiddleware(interrupt_on={"write_file": lambda args: args.get("path", "").endswith(".md")}),
    ],
...
| [`HumanInTheLoopMiddleware`](https://reference.langchain.com/python/langchain/agents/middleware/human_in_the_loop/HumanInTheLoopMiddleware) | Human review before publishing |
