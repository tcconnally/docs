...
- **Middleware** — each middleware bundles its own tools, prompt additions, and loop hooks into a single composable unit. Pass one or more to `middleware=` to extend the harness. See [Middleware overview](/oss/python/langchain/middleware/overview).
...
| **JavaScript REPL**—in-process scripting (QuickJS) | [`CodeInterpreterMiddleware`](/oss/python/deepagents/interpreters) | — |
| **Persistent shell**—shared working directory across turns | [`ShellToolMiddleware`](https://reference.langchain.com/python/langchain/agents/middleware/shell_tool/ShellToolMiddleware) | — |
| **Isolated sandbox**—code execution isolated from the host | [`SandboxBackend`](/oss/python/deepagents/sandboxes) | — |
...
As an agent runs, context accumulates—history, tool results, intermediate steps—and eventually overflows the context window. Context management attacks this at multiple levels: summarization compresses and offloads growing history before overflow hits; memory loads persistent instructions at the start of each session so knowledge carries across runs without re-stating it each time; skills use progressive disclosure so domain knowledge loads only when the task calls for it, avoiding context rot from irrelevant content sitting in the window throughout the run.

| Capability | How to add | In `create_deep_agent` |
|---|---|---|
| **History compression**—compresses history and offloads large results | [`SummarizationMiddleware`](https://reference.langchain.com/python/langchain/agents/middleware/summarization/SummarizationMiddleware) | ✓ |
| **Agent-triggered summarization**—agent controls when to compress | [`SummarizationToolMiddleware`](https://reference.langchain.com/python/deepagents/middleware/summarization/SummarizationToolMiddleware) | — |
| **Persistent memory**—AGENTS.md instructions loaded at startup | [`MemoryMiddleware`](https://reference.langchain.com/python/deepagents/middleware/memory/MemoryMiddleware) | ✓ if `memory=` |
| **On-demand skills**—domain knowledge loaded progressively | [`SkillsMiddleware`](https://reference.langchain.com/python/deepagents/middleware/skills/SkillsMiddleware) | ✓ if `skills=` |
...
| **Dynamic tool filtering**—trims tool list per model call | [`LLMToolSelectorMiddleware`](https://reference.langchain.com/python/langchain/agents/middleware/tool_selection/LLMToolSelectorMiddleware) | — |
...
| **Sync subagents**—isolated subtasks in their own context windows | [`SubAgentMiddleware`](https://reference.langchain.com/python/deepagents/middleware/subagents/SubAgentMiddleware) | ✓ |
| **Background subagents**—fire-and-forget tasks the main agent doesn't wait on | [`AsyncSubAgentMiddleware`](https://reference.langchain.com/python/deepagents/middleware/async_subagents/AsyncSubAgentMiddleware) | — |
